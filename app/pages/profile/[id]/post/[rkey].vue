...
<script setup lang="ts">
  ...
  const atUri: Ref<string> = ref('')

  const pds = ref<string>('')
  ...
  onMounted(async () => {
    ...
    pds.value = await bskyutils.getPDSEndpointByDID(did.value)
    profile.value = await bskyutils.loadProfile(pds.value, handleOrDid)
  })
</script>
